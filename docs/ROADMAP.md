...
Implementation note (2026-07-11): multi-profile support is wired end-to-end. LocalDataManager owns a profiles.json collection (max 3 profiles, active_profile.json selection, legacy single-profile/level_progress import) stored via the sqlite-first adapter, with the migration-readiness gate switched to a profiles_document_contract check and parity cases updated. GameStateManager is per-profile (set_active_profile, load/save by name, avg_accuracy recompute from NOTE_ATTEMPT grades). BattleManager stamps profile_name on NOTE_ATTEMPT and GAME_SESSION payloads. A BootScene launch router sends first-run users (no saved mic calibration) to MicrophoneCalibrationScene and returning users to ProfileSelectScene; ProfileSelectScene provides list/create/select/delete with max-3 and unique-name enforcement and routes to MainMenuScene; MainMenuScene exposes "Switch Profile" and a continuation label reflecting the active profile.
